...
@CapabilityDescription("Writes the contents of a FlowFile to a samba network location. "
        ...
        + "This is an extended variant of PutSmbFile whose Password property is not marked sensitive and "
        + "whose Username and Password properties support Expression Language against FlowFile attributes, "
        + "allowing credentials to be injected dynamically.")
...
public class PutSMBFileExtended extends AbstractProcessor {
    ...
    public static final PropertyDescriptor USERNAME = new PropertyDescriptor.Builder()
            ...
            .description("The username used for authentication. If no username is set then anonymous authentication is attempted. "
                    + "Supports Expression Language so the username can be supplied dynamically from a FlowFile attribute.")
            .required(false)
            .addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
            .expressionLanguageSupported(ExpressionLanguageScope.FLOWFILE_ATTRIBUTES)
            .build();
    public static final PropertyDescriptor PASSWORD = new PropertyDescriptor.Builder()
            .name("Password")
            .description("The password used for authentication. Required if Username is set. This property is not "
                    + "marked sensitive and supports Expression Language so the password can be supplied dynamically "
                    + "from a FlowFile attribute.")
            .required(false)
            .addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
            .expressionLanguageSupported(ExpressionLanguageScope.FLOWFILE_ATTRIBUTES)
            .sensitive(false)
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        // Username and Password support Expression Language; evaluate against the first FlowFile of the batch, which
        // by construction shares the same Hostname/Share (and therefore the same credentials) as the rest.
        final FlowFile credentialFlowFile = flowFiles.get(0);
        final String username = context.getProperty(USERNAME).evaluateAttributeExpressions(credentialFlowFile).getValue();
        final String password = context.getProperty(PASSWORD).evaluateAttributeExpressions(credentialFlowFile).getValue();
