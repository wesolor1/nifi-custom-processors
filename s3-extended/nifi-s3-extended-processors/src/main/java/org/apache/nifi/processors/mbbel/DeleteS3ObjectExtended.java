...
@CapabilityDescription("Deletes a file from an Amazon S3 Bucket. This is a drop-in variant of DeleteS3Object designed "
        + "for use with ListS3Extended and AWSCredentialsProviderControllerServiceExtended, where Access Key ID, "
        + "Secret Access Key and Endpoint Override URL can be supplied via Expression Language against incoming FlowFile "
        + "attributes (for example ${source_access_key_id}, ${source_endpoint_override}). Incoming FlowFile attributes "
        + "are published to the credentials service before the delete so they are available when credentials are resolved. "
        + "All other behavior is identical to the standard DeleteS3Object processor.")
