...
/**
 * Drop-in variant of {@link DeleteS3Object} for use with {@link ListS3Extended} and
...
@CapabilityDescription("Deletes a file from an Amazon S3 Bucket. This is a drop-in variant of DeleteS3Object designed "
        + "for use with ListS3Extended and AWSCredentialsProviderControllerServiceExtended, where Access Key ID and "
        + "Secret Access Key can be supplied via Expression Language against incoming FlowFile attributes (for example "
        + "${source_access_key_id}). Incoming FlowFile attributes are published to the credentials service before the "
        + "delete so they are available when credentials are resolved. All other behavior is identical to the standard "
        + "DeleteS3Object processor.")
@SeeAlso({DeleteS3Object.class, ListS3Extended.class, FetchS3ObjectExtended.class, PutS3ObjectExtended.class,
        AWSCredentialsProviderControllerServiceExtended.class})
public class DeleteS3ObjectExtended extends DeleteS3Object {
    ...
    /**
     * Stock DeleteS3Object calls this with {@code flowFile.getAttributes()} before talking to S3. Capture those attributes
    ...
    private final class FlowFileAwareCredentialsProvider implements AwsCredentialsProvider {
        ...
        public AwsCredentials resolveCredentials() {
            ...
            try {
                return DeleteS3ObjectExtended.super.getCredentialsProvider(context).resolveCredentials();
            } finally {
                AWSCredentialsProviderControllerServiceExtended.clearEvaluationAttributes();
            }
        }
